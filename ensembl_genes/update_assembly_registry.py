...
def update_assembly_sheet(existing_sheet_records,assembly_sheet,worksheet_name,accession):
...
if __name__ == '__main__':
  ...
  parser.add_argument('-as','--assembly_accession', help='Assembly identifier', required=True)
  ...
  parser.add_argument('-apw','--assembly_db_password', help='Password for assembly registry db', required=True)
  ...
  assembly_db_query = "update assembly set annotated_status = 'completed' where chain = '" + chain_version[0] + "' and version = " + chain_version[1]
  # use creds to create a client to interact with the Google Drive API
  ...
  update_registry_db(assembly_db_query,assembly_db_database,assembly_db_host,assembly_db_port,assembly_db_user,assembly_db_password)
