...
def test_overlap_empty_cds():
    gff = _gff(
        [{"Feature": "five_prime_UTR", "transcript_id": "TX1", "Start": 0, "End": 100}]
    )
    df = compute_cds_utr5_overlap(gff)
    assert df.empty
...
def test_overlap_gene_id_propagated():
    gff = _base_gff(
        "TX1", "MYGENE", cds_start=90, cds_end=200, utr5_start=0, utr5_end=100
    )
    df = compute_cds_utr5_overlap(gff)
...
def _splice_df(junction_types: list[str]) -> pd.DataFrame:
    rows = [
        {"junction_type": jt, "is_canonical": jt in {"GT-AG", "GC-AG", "AT-AC"}}
