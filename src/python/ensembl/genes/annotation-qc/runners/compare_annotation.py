...
def compare_annotations(file1, file2, output_path, name1="Funannotate", name2="EnsemblAnno"):
	...
	create_plots(stats1, stats2, genes1, genes2, name1, name2, output_path)

	return stats1, stats2, unique1, unique2, shared


def create_plots(stats1, stats2, genes1, genes2, name1, name2, output_path):
	...
	plt.tight_layout()
	plot_path = Path(output_path) / "annotation_comparison.png"
...
def main(gff_ref= Path, gff_test= Path, ref_name = str, test_name = str, gffcompare_bin = Path, output_path = Path):
	...
	compare_annotations(gff_ref, gff_test, output_path, str(ref_name), str(test_name))
