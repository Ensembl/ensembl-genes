...
def get_agat_path_from_config() -> Path:
    """
    Load AGAT path from software/software.json located next to this script.

    Expected format:
        { "agat": "/path/to/file" }
    """
    script_dir = Path(__file__).resolve().parent
    config_path = script_dir / "software" / "software.json"

    if not config_path.exists():
        raise FileNotFoundError(
            f"{config_path} not found. Provide --agat_path or create config."
        )

    with open(config_path) as f:
        config = json.load(f)

    if "agat" not in config:
        raise KeyError("Missing 'agat' key in software.json")

    return Path(config["agat"])
...
def main() -> None:
    ...
    agat_path = Path(args.agat_path) if args.agat_path else get_agat_path_from_config()
