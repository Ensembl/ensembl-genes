"""CDS metrics and genetic code utilities for translation validity assessment."""
...
def get_genetic_code(table_id: int) -> GeneticCode:
    ...
    try:
        ...
        raise ValueError(f"Unknown NCBI genetic code table: {table_id}") from exc
    return GeneticCode(
        table_id=table_id,
        name=table.names[0],
...
@dataclass
class CDSMetrics:  # pylint: disable=too-many-instance-attributes
    """
...
def compute_cds_metrics(
    seq_nt: str, genetic_code: GeneticCode = STANDARD_CODE
) -> CDSMetrics:
    """
    ...
    if length_of_cds < 3:
        ...
        stop_status = (
            "canonical" if stop_codon in genetic_code.stop_codons else "noncanonical"
        )
