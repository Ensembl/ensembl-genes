...
def write_content(assembly_info, output_dir, production_name, strain):
    """
    ...
    with open(output_dir / f"{production_name}_assembly.html", "w") as assembly_out:
        print(
            f"<p>The assembly for {strain} was generated as part of The Mouse Genomes Project "
            f"[https://www.mousegenomes.org/], additional strains can be found in Ensembl "
            f"[https://www.ensembl.org/Mus_musculus/Info/Strains].</p>"
    ...
    with open(output_dir / f"{production_name}_annotation.html", "w") as annotation_out:
        print(
            f"<p>Genome annotation was generated by mapping GENCODE M30 [https://www.gencodegenes.org/mouse/release_M30.html] "
            f"genes and transcripts via the Ensembl Human automated annotation system "
            f"[https://beta.ensembl.org/help/articles/human-genome-automated-annotation], supplemented by methods from the "
            f"Ensembl vertebrate annotation pipeline [https://beta.ensembl.org/help/articles/vertebrate-genome-annotation]. "
            f"Mapped GENCODE structures served as the primary evidence with gaps in the annotations filled using aligned short-read "
            f"transcriptomic data and full-length transcripts derived from PacBio IsoSeq long-read data."
            f"</p><p>In accordance with the "
            f"<a href=\"https://en.wikipedia.org/wiki/Fort_Lauderdale_Agreement\">Fort Lauderdale Agreement</a>, please check the publication "
            f"status of the genome/assembly before publishing any genome-wide analyses using these data.</p>",
            file=annotation_out
        )
...
if __name__ == "__main__":
    ...
    core_query = (
        ...
        "('assembly.accession', 'species.production_name', 'species.strain');"
    )
    ...
    write_content(assembly_info, output_dir, upper_production_name, strain)
