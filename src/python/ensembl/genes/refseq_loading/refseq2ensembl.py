...
NCBI_GROUPS = [
    ...
    "invertebrate",
    "fungi",
    "protozoa",  # ,"archaea", "bacteria", "viral"
]
...
def list_available_annotations(
    ...
    for group in groups_to_check:
        url = (
            f"https://ftp.ncbi.nlm.nih.gov/genomes/refseq/{group}/assembly_summary.txt"
        )
        try:
        ...
        for line in lines:
            ...
            clade_list.append(
                f"{species_binomial} [{'Downloaded' if downloaded else 'Not downloaded'}]"
            )
...
def convert_fna_headers(fna_in_path, assembly_report_path, fna_out_path):
    ...
    """
    # Build accession -> new_name map using the same rule as convert_gff_to_ensembl
    accession_to_name = {}
    with open(assembly_report_path, "r") as f:
        for line in f:
            ...
            cols = line.strip().split("\t")
            if len(cols) >= 7:
                seq_name = cols[0]  # MSCHRUN_CTG3
                assigned = cols[2]  # na
                refseq_acc = cols[6]  # NT_166462.1
                ...
                accession_to_name[refseq_acc] = new_name
...
def convert_gff_to_ensembl(
    gff_path, assembly_report_path, output_path=None, chrom_filter=None
):
    """
            ...
            if line.startswith("#"):
                ...
                if chrom_filter:
                    ...
                    if seq_id in chrom_filter:
                        ...
                        seq_id in refseq_to_name
                        and refseq_to_name[seq_id] in chrom_filter
                    ):
                        allow = True
                    if not allow:
                        continue
...
def load_to_ensembl_core(
    ...
    cur.execute(
        "INSERT INTO coord_system (name,version,rank,attrib) VALUES ('primary_assembly','',1,'default_version,sequence_level')"
    )
    cs_id = cur.lastrowid
    cur.executemany(
    ...
    cur.execute(
        "INSERT INTO analysis (logic_name,created,program) VALUES ('refseq_import',NOW(),'NCBI_RefSeq')"
    )
    ...
    for tid, cds_list in cds_segments.items():
        ...
        exons_tx_order = sorted(
            tx["exons"], key=lambda x: x["start"], reverse=(strand == -1)
        )
        ...
        if strand == 1:
            ...
            ordered_cds = sorted(
                cds_list, key=lambda c: c[1], reverse=True
            )  # by genomic end desc
    ...
    for gid, g in genes.items():
        ...
        first_tx = next(
            (t for t in transcripts if transcripts[t]["gene_id"] == gid), None
        )
    ...
    for tid, tx in transcripts.items():
        ...
        sorted_exons = sorted(
            tx["exons"], key=lambda x: x["start"], reverse=(tx["strand"] == -1)
        )
        coord_to_eid = {}
    ...
    for tid, cds_list in cds_segments.items():
        ...
        rank_map = {
            (ex["start"], ex["end"]): r for r, ex in enumerate(inserted_order, start=1)
        }
        ...
        for ex in genomic_sorted:
            if ex["start"] <= trans_start_pos <= ex["end"]:
                ...
                start_off = (
                    (trans_start_pos - ex["start"] + 1)
                    if strand == 1
                    else (ex["end"] - trans_start_pos + 1)
                )
                start_rank = rank_map[(ex["start"], ex["end"])]
            if ex["start"] <= trans_end_pos <= ex["end"]:
                end_off = (
                    (trans_end_pos - ex["start"] + 1)
                    if strand == 1
                    else (ex["end"] - trans_end_pos + 1)
                )
