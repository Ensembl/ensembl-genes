...
warnings.warn(
    "hprc_write_yaml.py is deprecated. Please use generate_project_yaml.py with --project hprc instead.",
    DeprecationWarning,
)
        ...
        if biosample_regex:
            ...
            for line in biosample_html:
                pop_desc_regex = re.search(
                    "Population Description\<\/th\>\<td\>([A-Za-z0-9 ]+)", line
                )
...
if __name__ == "__main__":
    ...
    for db in db_list:
        ...
        yaml = "- assembly: " + info_dict["assembly.name"] + "\n"
        if population:
            yaml += "  population: " + population + "\n"
        yaml += (
            "  assembly_accession: "
            + info_dict["assembly.accession"]
            + "\n"
        ...
        if clinvar_file:
            yaml += "  variants_clinvar: " + clinvar_file + "\n"
        if gnomad_file:
            yaml += "  variants_gnomad: " + gnomad_file + "\n"
        yaml += (
            "  ftp_dumps: "
            + ftp_base
            + "/"
            ...
            + info_dict["assembly.accession"]
            + "\n"
            "  rapid_link: https://rapid.ensembl.org/" + uc_prod_name + "/Info/Index\n"
        )
