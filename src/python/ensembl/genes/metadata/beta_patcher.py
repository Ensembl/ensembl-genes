...
def get_genome_by_uuid(genome_uuid: str) -> Optional[Dict]:
    ...
    try:
        results = adaptor.fetch_genomes_by_genome_uuid(
            genome_uuid=genome_uuid, status="All"
        )
        if results:
...
def _log_team_warnings(  # pylint: disable=too-many-locals, too-many-arguments
    validate_file,
    patch_file,
    genome_uuid: str,
...
def _write_validation_sql(  # pylint: disable= too-many-arguments
    validate_file,
    genome_uuid: str,
    attribute_name: str,
    escaped_value: str,
...
def write_metadata_patch_for_genome(  # pylint: disable=too-many-arguments
    validate_file,
    patch_file,
    genome_uuid: str,
