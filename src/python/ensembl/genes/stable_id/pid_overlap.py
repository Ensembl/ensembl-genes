            ...
            if columns[2] == 'CDS':
                ...
                if match:
                    ...
                    if protein_id not in protein_data:
                        ...
                        protein_data[protein_id]['Start'] = min(protein_data[protein_id]['Start'], int(columns[3]))
                        protein_data[protein_id]['End'] = max(protein_data[protein_id]['End'], int(columns[4]))
...
def compare_protein_sets(gff3_file_old, gff3_file_new, output_report):
    ...
    shared_proteins = pd.merge(proteins_old, proteins_new, on='ProteinID', suffixes=('_old', '_new'))

    # Compute proportions
    proportion_old = len(shared_proteins) / len(proteins_old) if len(proteins_old) > 0 else 0
    proportion_new = len(shared_proteins) / len(proteins_new) if len(proteins_new) > 0 else 0

    print(f"Proportion of proteins in the old GFF3 that have shared IDs: {proportion_old:.2%}")
    print(f"Proportion of proteins in the new GFF3 that have shared IDs: {proportion_new:.2%}")

    # Add columns for genomic locations in the report
    shared_proteins = shared_proteins[['ProteinID', 'Chromosome_old', 'Start_old', 'End_old', 'Strand_old',
                                       'Chromosome_new', 'Start_new', 'End_new', 'Strand_new']]

    # Save report
    shared_proteins.to_csv(output_report, index=False, sep='\t', quoting=csv.QUOTE_MINIMAL)
